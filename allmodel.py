from keras.callbacks import Callback,CSVLogger,ReduceLROnPlateau,EarlyStopping
...
#         Up = Activation("sigmoid")(Up)
        ...
        Up = Conv2D(self.nClasses, kernel_size=(1, 1),padding = 'same', activation = 'sigmoid')(Up)
    ...
    #     Patch_size = 224
        ...
        conv10 = Conv2D(self.nClasses, 1, activation = 'sigmoid',padding = 'same')(conv9)
   ...
   ## UNET_MINI
    def UNET_MINI(self):
        ...
        result = Conv2D(self.nClasses, 1, activation = 'sigmoid',padding = 'same')(conv5)
...
#         o = (ZeroPadding2D((1, 1)))(o)
        ...
        result = Conv2D(self.nClasses, 1, activation = 'sigmoid',padding = 'same')(o)
...
#         x = Lambda(self.interpolation, arguments={'shape': (size_before3[1], size_before3[2])})(x)
    ...
    def DEEPLABV3plus_SERESNEXT(self):
        inputs=self.img_input
        SERESNEXT_input, levels = self.ResNeXt50_head2()
        [f1, f2, f3, f4, f5] = levels
#         print(K.int_shape(f3),K.int_shape(f4),K.int_shape(f5))
        #F3 64,F4=F5=32
        b4 = GlobalAveragePooling2D()(f5)
...
#         x = Lambda(self.interpolation, arguments={'shape': (size_before3[1], size_before3[2])})(x)
        ...
        self.DEEPLABV3plus_SERESNEXT=Model(inputs=inputs,outputs=outputs)
        self.DEEPLABV3plus_SERESNEXT.compile(optimizer=self.OPTIMIZER, loss=self.LOSS, metrics=self.METRICS)
        return self.DEEPLABV3plus_SERESNEXT